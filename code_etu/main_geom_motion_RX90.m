...
Bgroup       =   uibuttongroup('Parent',numfig, ...
    'Units', 'normalized',...
    'Position',[0.0 0.10 0.06 0.05],...
    'Title',{''},...
    'BorderType', 'none');
h_IVKGo    =   uicontrol('Parent',Bgroup,...
    'Units', 'normalized',...
    'Position',[0 0 1 1],...
    'String', 'IVK Go',...
    'Callback',@IVKGoCallback,'interruptible','on','busyaction','cancel');

% "Stop" Button
Bgroup       =   uibuttongroup('Parent',numfig, ...
    'Units', 'normalized',...
    'Position',[0.0 0.15 0.06 0.05],...
    'Title',{''},...
    'BorderType', 'none');
h_StopGo    =   uicontrol('Parent',Bgroup,...
    'Units', 'normalized',...
    'Position',[0 0 1 1],...
    'String', 'Stop',...
    'Callback',@StopCallback,'interruptible','off','busyaction','cancel');

% "qGoal" fields
h_qGoalT = uicontrol('Parent',numfig,'style','text','Units','normalized','String','Goal Configuration:','Position',[0.08 0.01 0.13 0.03]);
h_qGoalF = uicontrol('Parent',numfig,'style','edit','Units','normalized','String',mat2str(round(q,2)),'Position',[0.22 0.01 0.2 0.03],'Callback',@getQCallback);
set([h_qGoalT, h_qGoalF],'HorizontalAlignment','left');

% "numSol" fields
h_numSolT = uicontrol('Parent',numfig,'style','text','Units','normalized','String','Solution Number IK:','Position',[0.01 0.25 0.13 0.03]);
h_numSolF = uicontrol('Parent',numfig,'style','edit','Units','normalized','String',mat2str(IndSol),'Position',[0.12 0.25 0.05 0.03],'Callback',@getIndSolCallback);
set([h_numSolT, h_numSolF],'HorizontalAlignment','left');


% "EE Goal" fields
h_EEpGoalT = uicontrol('Parent',numfig,'style','text','Units','normalized','String','Goal EE Position:','Position',[0.08 0.11 0.13 0.03]);
h_EEpGoalF = uicontrol('Parent',numfig,'style','edit','Units','normalized','String',mat2str(round(EEpGoal,2),3),'Position',[0.22 0.11 0.12 0.03],'Callback',@getEEpCallback);

h_EEoGoalT = uicontrol('Parent',numfig,'style','text','Units','normalized','String','Goal EE Orientation:','Position',[0.08 0.07 0.13 0.03]);
h_EEoGoalF = uicontrol('Parent',numfig,'style','edit','Units','normalized','String',mat2str(round(EEoGoal,2),3),'Position',[0.22 0.07 0.25 0.03],'Callback',@getEEoCallback);
...
h_T = uicontrol('Parent',numfig,'style','text','Units','normalized','String','Current EE Pose :','Position',[0.8 0.1 0.19 0.20]);
